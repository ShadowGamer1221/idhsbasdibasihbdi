...
const config = {
    description: 'Suspends a user in the Roblox group.',
    aliases: [],
    ...
    slashOptions: [
        {
            type: 'STRING',
            name: 'username',
            description: 'The username of the user you want to suspend.',
            required: true
        },
        {
            type: 'STRING',
...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if(rankInGroup === client.config.suspendedRank) {
        ...
        let rankingTo = client.config.suspendedRank;
        ...
        let rankingInfo;
        try {
            rankingInfo = await roblox.setRank(client.config.groupId, id, client.config.suspendedRank);
        } catch (err) {
        ...
        embed.setDescription(`**Success!** Suspended ${username}.`);
    ...
    runInteraction: async (client, interaction, args) => {
        ...
        let rankInGroup = await roblox.getRankInGroup(client.config.groupId, id);
        if(rankInGroup === client.config.suspendedRank) {
            embed.setDescription(`That user has already been suspended.`);
            embed.setColor(client.config.colors.error);
            embed.setAuthor(interaction.user.tag, interaction.user.displayAvatarURL());
            return interaction.reply({ embeds: [embed] });
        }
